...
def get_sick_model_family(model: str):
    if model.startswith("lms1"):
        return "lms1xx"
    if model.startswith("tim5"):
        return "tim5xx"
    if model.startswith("mrs1"):
        return "mrs1xxx"
...
def get_robosense_model_family(model):
    if model == "airy":
        return model

    raise RuntimeError(
        f"Robosense '{model}' lidar is not supported by romea_lidar_description package. "
        "Please check your configuration or contribute to support this sensor."
    )
...
def get_lidar_family(type, model):
    if type == "sick":
        ...
        return get_robosense_model_family(model)
    else:
        raise RuntimeError(
...
def sick_lms1xx_specifications(model, rate, resolution):
    ...
    if rate is not None:
        ...
        if resolution in specifications["azimut_angle_increment"].values():
            ...
            raise ValueError("Resolution " + str(resolution) + " is not available for lms1xx lidar")
...
def ouster_os1_32_specifications(model, rate, resolution):
    specifications = get_lidar_family_specifications("ouster", model)
...
def robosense_airy_specifications(model, rate, resolution):
    specifications = get_lidar_family_specifications("robosense", model)

    if rate is not None and rate != specifications["rate"]:
        raise ValueError(f"Unknown rate '{rate}', available: {specifications['rate']}")

    if resolution is not None and resolution != specifications["resolution"]:
        raise ValueError(
            f"Unknown resolution '{resolution}', available: {specifications['resolution']}"
        )

    # Because there is no choices in the yaml file, the dictionary can be returned as is
...
def get_lidar_geometry_file_path(type, model):
    ...
    model = get_lidar_family(type, model)
